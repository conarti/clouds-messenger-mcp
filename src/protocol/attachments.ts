 ...
 *
 * ФОРМА НАБЛЮДЕНА ЖИВЬЁМ ТОЛЬКО У `image` (findings.md, P2): внутреннее событие типа
 * `image` несёт `payload` с полями `file`, `file_name`, `file_size`, `file_hash`,
 * `file_mime_type`, `chunk_size`, `file_encryption_algo`, `file_preview`,
 * `file_preview_width`, `file_preview_height`, `blur_preview_file`, `file_id`. Событие
 * `file` считается устроенным так же ПО АНАЛОГИИ, живой пробой оно не наблюдалось.
 *
 * ПРИЗНАК ВЛОЖЕНИЯ ЭТО `file_id`, А НЕ СПИСОК ТИПОВ. Именно поэтому вывод по аналогии
 * ничего здесь не решает: закрытым набор файловых типов никто не объявлял, и проверка по
 * типу теряла бы вложение молча на первом же новом виде события.
 *
