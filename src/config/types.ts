...
export interface WsConfig {
  ...
  /**
   * Простой, после которого сокет закрывается сам. Поле не про экономию ресурсов, а про
   * присутствие: пока сокет открыт, мессенджер показывает человека в сети, и висящее
   * соединение сообщает коллегам неправду о том, что он работает. Следующий запрос
   * поднимает соединение заново, поэтому цена закрытия это одна повторная авторизация
   * сокета. Ноль ВЫКЛЮЧАЕТ авто-закрытие.
   */
  idleCloseMs: number;
}
